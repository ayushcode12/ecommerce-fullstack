...
const AdminProducts = () => {
  ...
  const onDeleteCategory = async () => {
    ...
    const categoryToDelete = categories.find(
      (category) => String(category.id) === String(productForm.categoryId)
    )

    const shouldDelete = window.confirm(
      `Delete category "${categoryToDelete?.name || "this category"}"?`
    )
    if (!shouldDelete) return

    try {
      setDeletingCategory(true)
      await api.delete(`/categories/${productForm.categoryId}`)

      setCategories((prev) =>
        prev.filter((category) => String(category.id) !== String(productForm.categoryId))
      )
          ...
          <form onSubmit={onSaveProduct} className="mt-4 space-y-4">
            ...
            <label className="field-group">
              ...
              <div className="mt-2 flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
                ...
                <button
                  type="button"
                  disabled={deletingCategory || !productForm.categoryId}
                  onClick={onDeleteCategory}
                  className="inline-flex items-center justify-center gap-1 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                >
                  <Trash2 size={12} />
                  {deletingCategory ? "Deleting..." : "Delete selected"}
                </button>
              </div>
            </label>
